...
class guass_decoder:
    ...
    def decode(self,syndrome):
        ...
        g = self.ms_decoder.z3_decode(g_syn,order=5)
...
def test_decoder(num_trials,surface_code,p,ourdecoder):
    ...
    print(f"BP error rate: {bp_error_rate * 100:.3f}%")
    print(f"BP+OSD error rate: {bposd_error_rate * 100:.3f}%")
    # print(f"UF Success rate: {uf_error_rate * 100:.2f}%")
    print(f"Our error rate: {our_error_rate * 100:.3f}%")
...
if __name__ == "__main__":
    ...
    test_decoder(num_trials=100,surface_code=surface_code,p=p,ourdecoder=ourdecoder)
