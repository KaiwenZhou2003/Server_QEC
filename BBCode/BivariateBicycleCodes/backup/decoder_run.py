...
for trial in range(num_trials):
    ...
    end_time = time.time()
    print(f"Z error, time for one round = {(end_time-start_time)/(num_cycles+2)}")
    ...
    if ec_resultZ:
        ...
        end_time = time.time()
        print(f"X error, time for one round = {(end_time-start_time)/(num_cycles+2)}")
